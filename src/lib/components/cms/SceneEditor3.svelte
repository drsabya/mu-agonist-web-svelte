<script lang="ts">
	...
	type SceneItem = {
		...
		// 🔹 NEW: base opacity (0–100). Rendered always; 'anim_opacity' is used in FK mode.
		opacity?: number | null;
		...
		anim_opacity?: number | null; // FINAL KEYFRAME absolute opacity (0-100)
	...
	function ensureAnimDefaultsForAll() {
		items = items.map((it) => ({
			...
			// 🔹 Default FK opacity to current displayed (base) opacity
			anim_opacity:
	...
	// 🔹 NEW: display opacity (%) depending on mode
	...
	async function saveItem() {
		...
		try {
			...
			const body: Record<string, any> = {
				...
				z_index: it.z_index,
				opacity: clampPercent(isFinite(it.opacity as number) ? (it.opacity as number) : 100), // 🔹 save base opacity
				role: it.role,
				...
				anim_opacity: it.anim_opacity ?? null, // 🔹 save FK opacity
	...
	function saveSettingsFromForm(fd: FormData) {
		...
		// 🔹 Animation settings (ONLY when not a slider scene and FK mode is on) — includes FK opacity
